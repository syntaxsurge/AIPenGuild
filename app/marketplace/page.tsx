...
import { NFT_CATEGORIES, NFT_RARITIES } from '@/lib/metadata-constants'
...
export default function MarketplacePage() {
  ...
  return (
    <main className='relative flex min-h-screen bg-white text-foreground dark:bg-gray-900'>
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-80 transform transition-transform duration-300 ease-in-out ${
          sidebarOpen ? 'translate-x-0' : '-translate-x-full'
        } md:relative md:translate-x-0`}
      >
        <Card className='h-full rounded-none border-r border-border'>
          ...
          <CardContent className='space-y-6 overflow-auto px-4 py-4'>
                ...
                <AccordionContent>
                  <div className='mt-2 space-y-4'>
                    ...
                    <div className='mt-2 flex justify-between text-xs text-muted-foreground'>
                      <span>
                        {tempPriceRange[0]} {currencySymbol}
                      </span>
                      <span>
                        {tempPriceRange[1]} {currencySymbol}
                      </span>
                    </div>
